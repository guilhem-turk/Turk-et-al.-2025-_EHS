...
plot_fdc.a <- ggplot(df_fdc) + 
  geom_ribbon(data=df_fdc.quantiles, aes(x = epf, ymin = q_down, ymax =q_up), fill="skyblue", alpha = 0.5) +
  geom_line(data=dff, aes(x=index2, y=sec_y_transform(q)), color="darkblue", linewidth=0.3) + 
  geom_line(data=df_fdc.mean, aes(x=epf, y=q), color="darkgrey", linewidth=0.3) +
  geom_line(aes(x=epf, y=q), color="black", linewidth=0.3) +
  geom_line(data=subset(df_fdc.yy, year == "2021"), 
            aes(x=epf, y=q, group=year), linewidth=0.3, linetype="dashed") +
  geom_line(data=subset(df_fdc.yy, year == "2011"), 
            aes(x=epf, y=q, group=year), linewidth=0.3, linetype="dashed") +
...
plot_fdc <- ggarrange(plot_fdc.a, plot_fdc.b,
                      labels = c("(a)","(b)"), font.label = list(face = "plain", size=10),
...
ddf_physio <- subset(dff_physio, geology=="Sandstone and conglomerates")
ddf_physio <- rename(ddf_physio[,-2], sandstone = value)


ddf <- merge(ddf, ddf_physio)





plot_boxplots.a <- ggplot(ddf.annual) + 
  geom_hline(aes(yintercept=median(P, na.rm=T)), linetype="dashed", lwd=0.3) +
  geom_violin(aes(x=station, y=P, group=station), fill="skyblue", scale = "width", size = 0.2, alpha=0.7) +
  geom_boxplot(aes(x=station, y=P, group=station), width=0.1, outlier.shape = NA, lwd=0.3) +
  theme_bw() + labs(x="", y=expression(paste(P[a]," [mm]"))) +
  theme(panel.grid.major = element_line(linetype = "dashed"), 
        panel.grid.minor = element_blank(),
        strip.background = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1),
        text=element_text(size=7)) + facet_grid(~station2,  scales ="free_x", space = "free") 




plot_boxplots.b <- ggplot(ddf) + 
  geom_hline(aes(yintercept=median(D, na.rm=T)), linetype="dashed", lwd=0.3) +
  geom_violin(aes(x=station, y=D, group=station, fill=sandstone), scale = "width", size = 0.2, alpha=0.7) +
  geom_boxplot(aes(x=station, y=D, group=station), width=0.1, outlier.shape = NA, lwd=0.3) +
  scale_fill_gradientn(colours = c("white","#FFBA08","#F48C06")) +
  theme_bw() + labs(x="", y="D [mm]", fill="Fraction of\nsandstone and\nconglomerates [%]") +
  theme(panel.grid.major = element_line(linetype = "dashed"), 
        panel.grid.minor = element_blank(),
        strip.background = element_blank(),
...
plot_boxplots <- ggarrange(plot_boxplots.a + rremove("xlab") + rremove("x.text") + rremove("x.ticks"), plot_boxplots.b,
                      labels = c("(a)","(b)"), font.label = list(face = "plain", size=8),
...
ggsave(plot_fdc, path="figures", filename = "fdc_new2.png",
