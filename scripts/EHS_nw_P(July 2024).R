...
plot_fnw_profile <- ggplot() + 
  ...
  geom_point(data=fnw_profile.nr_p, aes(x=P+1, y=qpFnew, color=station2, shape="Non-robust"), size=2) +
  geom_errorbar(data=fnw_profile_p, aes(x=P, y=qpFnew, ymin=qpFnew-qpFnew_se, 
                                        ymax=qpFnew+qpFnew_se, color=station2), width=0, linewidth=0.6) +
  geom_point(data=fnw_profile_p, aes(x=P, y=qpFnew, color=station2, shape="Robust"), fill="white", size=2, stroke=1) +
  geom_text(data = facet_labels, aes(x = P, y = qpFnew, label = station, color=station2), 
            hjust = 0, vjust = 1, size = 3) + 
  theme_bw() + labs(x=expression(paste("P [mm (2 weeks)", .^{-1},"]")), y="Fnew [-]", 
                    color="Bedrock geology", shape="Method") +
  facet_wrap(~station, ncol=3) + 
  scale_y_continuous(sec.axis = ~.) +
  scale_color_manual(values=c("#264653","#2a9d8f","darkblue","#e9c46a")) +
