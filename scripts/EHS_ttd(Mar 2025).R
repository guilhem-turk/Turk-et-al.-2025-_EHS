...
ddf <- merge(ddf, ddf_physio[,-c(2,3)])
ddf2 <- merge(ddf2, ddf_physio[,-c(2,3)])
...
plot <- ggplot(ddf) + 
  ...
  geom_errorbar(aes(x=lag*2+1, y=ttd, ymin=ttd-ttd_se, ymax=ttd+ttd_se, color=geo_ratio), 
                width=0, linewidth=0.4) +
  geom_point(aes(x=lag*2+1, y=ttd, color=geo_ratio, shape=name), fill="white", stroke=0.7, size=1) +
  geom_text(data = facet_labels, aes(x = lag*2+1, y = ttd, label = station), 
            hjust = 0, vjust = 1, size = 3, color = "black") + 
  geom_text(data = facet_labels2, aes(x = lag*2+1, y = ttd, label = station2), 
