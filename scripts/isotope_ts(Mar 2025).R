...
dff_isotopes2 <- dff_isotopes2 %>%  mutate(station2=recode(station2, "Schistose"="Weathered layer", "Marly"="Impermeable layer",
                                       "Permeable layer interface"="Permeable layer interface", "Aggregated"="Aggregated"))

dff_isotopes2$station2 <- factor(dff_isotopes2$station2, levels = c("Weathered layer", "Impermeable layer", "Permeable layer interface", "Aggregated"))
...
dual_plot <- ggplot(rain_isotopes2) + 
...
dff_isotopes2 <- dff_isotopes2 %>%  mutate(station2=recode(station2, "Weathered layer" = "WL", "Impermeable layer" = "IL",
                                                           "Permeable layer interface"="PLI", "Aggregated"="AG"))

dff_isotopes2$station2 <- factor(dff_isotopes2$station2, levels = c("WL", "IL", "PLI", "AG"))
...
dff_isotopes2 <- merge(dff_isotopes2, ddf_physio[,-c(2,3)])
...
plot_stream <- ggplot(subset(dff_isotopes2, P0 <= 15)) +
  ...
  geom_point(aes(x=date, y=dO.y, color=geo_ratio), size=0.6, alpha=0.8) +
  geom_point(data=subset(dff_isotopes2, P0 > 15), aes(x=date, y=dO.y), shape=21, size=0.6, alpha=0.7) +
  geom_text(data = facet_labels, aes(x=date, y=dO.y, label=c("Weathered layer (WL)","Impermeable layer (IL)",
                                                             "Permeable layer interface (PLI)","Aggregated (AG)")), 
  ...
  theme_bw() + facet_wrap(~station2, ncol=2) + 
...
plot_stream.bis <- ggplot(dff_isotopes2) + 
  ...
  geom_boxplot(aes(x=station, y=dO.y, group=station, fill=geo_ratio), width=0.4, lwd=0.3, fatten=1,
               outlier.shape = NA) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        text=element_text(size=10),
        legend.title=element_text(size=8)) + ylim(-10,-5) +
  facet_grid(~station2,  scales ="free_x", space = "free") 
...
plot_stream.comb <- ggarrange(plot_stream, plot_stream.bis, 
                              ncol=1, heights=c(1.5,1), labels = c("(a)", "(b)"), 
...
ggsave(plot_stream.comb, path="figures", filename = "stream_isotopes_new.png",
       device = ragg::agg_png, dpi=350,
