...
new_labels = c("area"="Area [km2]","forested"="Forest [%]","grasslands"="Grassland [%]",
               "agricultural"="Agriculture [%]","elevation"="Elevation [m a.s.l.]","sandstone"="Sandstone [%]",
               "marls"="Marls [%]","shists"="Schists [%]","q"="qa [mm]","P"="Pa [mm]","PET"="PETa [mm]","Smax"="Smax [mm]")
...
plot_physio <- ggplot(subset(ddf_alt, profile=="q20")) +
  geom_point(aes(x=value, y=qpFnew*100, shape=station2), size=1) +
  stat_cor(aes(x=value, y=qpFnew*100, group=predictor, 
               label = paste0("`", "r", "`~`=`~", ..r.., "~`,`~`p =`~", 
                              sprintf("%.3f", as.numeric(..p..)))),
  ...
  theme_bw() + labs(x="", y=expression(paste(F[new]," [-]")), 
                    shape="Hydro-lithological category") +
...
ddf2 <- rename(ddf2, "Area [km2]"="area", "Forest [%]"="forested","Grassland [%]"="grasslands", "Agriculture [%]"="agricultural",
       "Elevation [m a.s.l.]"="elevation", "Sandstone [%]"="sandstone", "Marls [%]"="marls", "Schists [%]"="shists",
       "qa [mm]"="q", "Pa [mm]"="P", "PETa [mm]"="PET", "Smax [mm]"="Smax")
...
plot_cor <- {corrplot(M, p.mat = testRes$p, order="hclust", type="upper", diag=FALSE, method = 'circle', sig.level = c(0.001, 0.01, 0.05), insig = "label_sig",
